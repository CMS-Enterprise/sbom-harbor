    ...
    match env::var("SBOM_SCORECARD") {
        Ok(sbom_scorecard) => {
            ...
            let scorecard_obj: SbomScorecard = serde_json::from_str(&raw_scorecard).unwrap_or({
                let empty_row = RowData{ ratio: 0.0, reasoning: String::new(), max_points: 0 };
                SbomScorecard { compliance: empty_row.clone(), package_identification: empty_row.clone(), package_versions: empty_row.clone(), package_licenses: empty_row.clone(), creation_info: empty_row.clone(), total: empty_row, metadata: SbomScorecardMetadata{total_packages:0} }
            });
...
pub fn compare_sbom_scorecards(sbom_1_path: String, sbom_2_path: String) -> Result<String, Error> {
    ...
    match  (scorecard_1_result, scorecard_2_result){
        (Ok(scorecard_1), Ok(scorecard_2)) => {
            let precision = 0;
            let scorecard_1_details = format!("Scorecard 1:({})\n=> Has a total score of {:.2$}/100", sbom_1_path, 100.0 * scorecard_1.total.ratio, precision);
            let scorecard_2_details = format!("Scorecard 2:({})\n=> Has a total score of {:.2$}/100", sbom_2_path, 100.0 * scorecard_2.total.ratio, precision);
        ...
        (Ok(_), Err(error)) => {
            ...
            Err(Error::SbomScorecard(format!("Failed to compare scorecards due to errors: {}", error_string)))
        },
        (Err(error), Ok(_)) => {
