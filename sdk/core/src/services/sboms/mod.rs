...
pub fn syft(source_path: &str, name: &str, version: &str) -> Result<String, Error> {
        ...
        .output()
    {
        Ok(output) => output,
        Err(err) => {
            return Err(Error::Sbom(format!("error executing syft cli: {}", err)));
        }
    };
    ...
    if !&output.status.success() {
        match String::from_utf8(output.stderr) {
            Ok(stderr) => {
                Error::Sbom(format!("error generating SBOM: {}", &stderr));
            }
            Err(err) => {
                return Err(Error::Sbom(format!(
                    "error formatting syft stderr: {}",
                    &err
                )));
            }
        };
    ...
    if output.stdout.is_empty() {
        return Err(Error::Sbom("syft generated empty SBOM".to_string()));
    };

    match String::from_utf8(output.stdout) {
        Ok(sbom) => Ok(sbom),
        Err(e) => Err(Error::Sbom(format!("error reading stdout: {}", e))),
    }
}
