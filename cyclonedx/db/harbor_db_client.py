...
class HarborDBClient(object):
    ...
    def _recurse(model: T, func: Callable) -> None:
        ...
        for entity_type, child_model_objs in items:
    ...
    def create(
        ...
        try:
            ...
            err_msg = f"Error creating {type(model)}: {str(err)}"
            raise DatabaseError(err_msg)

        if recurse and model.has_children():
            HarborDBClient._recurse(
                model=model,
                func=self.create
            )
    ...
    def update(
        ...
        try:
            ...
            err_msg = f"Error updating {type(model)}: {str(err)}"
            raise DatabaseError(err_msg)
    ...
    def delete(
        ...
        try:
            ...
            err_msg = f"Error deleting {type(model)}: {str(err)}"
            print(err_msg)
            raise DatabaseError(err_msg)
            ...
            for child_type in parent.get_child_types():
                try:
                    results: dict = self.table.query(
                        KeyConditionExpression=equals_clause & sort_key.begins_with(child_type),
                    )
    ...
    def get(
        ...
        try:
            ...
            err_msg = f"Error getting {type(model.entity_key)}: {str(err)}"
            print(err_msg)
            raise DatabaseError(err_msg)
